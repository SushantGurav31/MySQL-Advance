...
select count(*) as total_laotops from Laptop;
...
select * from Laptop where Graphics like '%Intel%';
...
Select Min(Price) as min_price, max(Price) as max_price from Laptop;

-- 	13.	Count how many laptops belong to each OS.
select OS, count(*) as total_laptops from Laptop group by OS;
...
select Ram, count(*) as total_laptops from Laptop group by Ram;

-- 	16.	Find the total number of laptops with SSD ≥ 512 GB.
select count(*) as total_laptops from laptop where SSD >= 512;
...
select Generation,
count(*) as total_laptop from Laptop group by Generation having count(*) > 2;

-- 	20.	Find the highest-rated laptop.
select Rating from Laptop order by Rating desc limit 1;

--  21.	Find laptops whose model name contains “Gaming”.
select * from Laptop where Model like '%Gaming%';

-- 	22.	Display laptops with AMD Ryzen processors.
select * from Laptop where Generation like '%AMD Ryzen%';
...
select * from Laptop where Price between 40000 and 70000;

-- 	27.	Find laptops with Quad Core or higher processors.
select * from Laptop where Generation >= 4;
...
select * from Laptop where Display = 15.6;
