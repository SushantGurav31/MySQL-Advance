create database AI_Developer_Data;
...
select Cognitive_Load, avg(Errors) as avg_errors from AI_Developer group by Cognitive_Load;
...
select Coffee_Intake, count(*) as total from AI_Developer group by Coffee_Intake;

-- Group by Stress_Level ranges (e.g. 0–50, 51–80…) and calculate average Sleep_Hours.
select Stress_Level, avg(Sleep_Hours) as avg_hours from AI_Developer group by Stress_Level;

-- Group by AI_Usage_Hours and compute average Task_Success_Rate.
select AI_Usage_Hours, avg(Task_Success_Rate) as avg_hours from AI_Developer group by AI_Usage_Hours;

-- Count number of developers grouped by Hours_Coding > 8 (using CASE).
select Hours_Coding,  count(*) as developers from AI_Developer group by Hours_Coding having Hours_Coding > 8;

-- Group by Cognitive_Load and show only groups having more than 20 rows.
select Cognitive_Load, count(*) as total from AI_Developer group by Cognitive_Load having count(*) > 20;

-- Calculate Bugs_Remaining = Bugs_Found – Bugs_Fixed for every developer.
select * , (Bugs_Found - Bugs_Fixed) as Bugs_Remaining from AI_Developer;
